...
public class AddThymeleafSpringSecNamespace extends BaseThymeleafNamespaceAction {
    ...
    public AddThymeleafSpringSecNamespace() {
        super("HtmlThymeleafSpringSecurity");
    }

    @Override
    public void actionPerformed(@NotNull AnActionEvent anActionEvent) {
        logger.info("AddThymeleafSpringSecNamespace.actionPerformed(...)");
        PsiFile selectedFile = getSelectedPropertiesFile(anActionEvent, true, getGroupId());
                ...
                ApplicationManager.getApplication().runWriteAction(() -> {
                    try {
                        ...
                        if (htmlElement != null && !htmlElement.hasAttr(THYMELEAF_SPRING_SECURITY_NAMESPACE.getAttribute())) {
                            ...
                            htmlElement.attr(
                                    THYMELEAF_SPRING_SECURITY_NAMESPACE.getAttribute(),
                                    THYMELEAF_SPRING_SECURITY_NAMESPACE_URI);
                        }
                        ...
                        Bus.notify(new Notification(getGroupId(), "File transformed", "Thymeleaf Namespace '"+THYMELEAF_SPRING_SECURITY_NAMESPACE.getAttribute()+"=\""+ THYMELEAF_SPRING_SECURITY_NAMESPACE_URI +"\"' added to html tag successfully.", NotificationType.INFORMATION));
                    } catch (Exception e) {
